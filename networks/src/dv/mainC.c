...
/**
 ***********************************************************************************
 ******** Linked List - Util Functions - Use to batch send2Neighbour requests ******
 ***********************************************************************************
 **/
...
int getNextNodeInRoute(int src, int dest, struct forwardingTable ft){
	if(src==dest){
		return -1;
	}else{
		return ft.nextHopToNodes[dest];
	}
}
...
void routeTraffic(int src,int dest, struct forwardingTable ft){
	...
	repeatNodesCount[src] += 1;

	while(srcNode!=destNode){
		printf(">%d", nodeFt.nextHopToNodes[destNode]);
		repeatNodesCount[nodeFt.nextHopToNodes[destNode]]+=1;
		if(repeatNodesCount[nodeFt.nextHopToNodes[destNode]]<=1){
...
void DvFtUpdateUsingBF(struct distance_table* dt, struct forwardingTable* ft, int node){
		...
		if(node !=i){
			...
			for(int j=0;j<num_nodes;j++){
				if(node==j){
					...
					//simple BellMan equation
					int costNtoJ=link_costs[node][j];
					int costJtoI=dt->costs[j][i];
					int costNtoI=dt->costs[node][i];
					if(costNtoJ!=-1 && costJtoI!=-1){
						//Bellman Ford Check
						//is new dist smaller than orig cost
						if(	costNtoI==-1 || (costNtoI > costNtoJ+costJtoI)){
							int newPathCost=costNtoJ+costJtoI;
							dt->costs[node][i]=newPathCost;
							if(newPathCost != ft->costToNodes[i]){
								setNodeAsUpdated(node);
							}
							ft->costToNodes[i]=dt->costs[node][i];
							ft->nextHopToNodes[i]=j;
						}else if((i==j) && (costNtoI==(costNtoJ+costJtoI))){
...
void updateLinkCosts(){
	...
	for(int i=0;i<trafficLinesCount;i++){
		...
		int alreadyVisitedNodes[num_nodes];
		memset(alreadyVisitedNodes,0,sizeof(alreadyVisitedNodes));

		//now visit that node
		alreadyVisitedNodes[src]+=1;
		nextNode=getNextNodeInRoute(src,dest,fts[src]);
		
		//store traversed path - used for loop detection
		int traversedPath[100]={-1};
		int traversedPathIdx=0;
		traversedPath[traversedPathIdx]=src;

		while(nextNode!=-1){
			alreadyVisitedNodes[nextNode]+=1;
			
			traversedPathIdx++;
			traversedPath[traversedPathIdx]=nextNode;

			if(alreadyVisitedNodes[nextNode]>1){
				loop=true;
				// use traversed path info to undo packets
				break;
			}
			//update link cost
			link_costs[src][nextNode]+=pcktCount;
			link_costs[nextNode][src]+=pcktCount;
			src=nextNode;
			nextNode=getNextNodeInRoute(src,dest,fts[src]);
		}
		...
		if(loop){
			loop=false;
			int loopNodeVal=traversedPath[traversedPathIdx];
			int loopStartIdx=0;

			while(traversedPath[loopStartIdx]!=loopNodeVal){
				loopStartIdx++;
			}

			for(int idx=loopStartIdx;idx<traversedPathIdx-1;idx++){
				link_costs[traversedPath[idx]][traversedPath[idx+1]]-=pcktCount;
				link_costs[traversedPath[idx+1]][traversedPath[idx]]-=pcktCount;
			}
		}
	}
}
...
void rtinit(struct distance_table *dt,struct forwardingTable *ft, int node, int *link_costs, int num_nodes)
{
	...
	//update the DT table for a given node 'node'
	for(int i=0;i<num_nodes;i++){
...
void main(int argc, char *argv[])
{
	...
	repeatNodesCount = (int* )malloc(num_nodes*sizeof(int));
	...
	while (1) 
	{
		...
		while(1){
			...
			if (eventptr->evtype == FROM_LAYER2 ){
				if(k==1){
					...
					dts[eventptr->eventity].costs[eventptr->rtpktptr->sourceid]=eventptr->rtpktptr->mincost;
				}
			}else {
		...
		//call BF algo
		for(int i=0;i<num_nodes;i++){
			DvFtUpdateUsingBF(&dts[i],&fts[i],i);
		}
